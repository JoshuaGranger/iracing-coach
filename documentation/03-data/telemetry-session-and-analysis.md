...
| `TD-027A` | Incident-point events MUST retain their individual lap, point increment, recorded session time, before/after counter values, and source-channel provenance when supplied. An explicit zero-point event MUST remain distinct from an absent event. The ordinary iRacing cumulative incident counters expose point increases, not every contact or loss-of-control classification; therefore a displayed lap total of `x0` means no point increase was recorded and MUST NOT be interpreted as proof that no zero-point contact occurred. |
...
Current coordinator mapping preserves individual nonnegative incident-point events, including an explicit zero when a source actually supplies one, instead of prematurely collapsing them into one lap total. The Race Analysis rail may sum those events for its compact `xN` badge while retaining event rows for the tooltip. Current native iRacing counter-derived analysis still cannot discover a contact that produces no observable counter event; `x0` communicates the recorded point total, not an exhaustive incident classification.
