...
| `GAP-002` | Closed in 0.10.0: registry and working Track View, Setup Comparison, and Starting Tune behaviors are aligned and tested. | Capability truth must agree with user-visible reality and automated gating. | Reopen only with a named source/registry mismatch. |
| `GAP-003` | Closed in development in 0.10.0; packaged and real baseline acceptance remain quality gates rather than missing product behavior. | Starting Tune is now first-class inside Setups. | Context/source/package/baseline UI, package mapper, Q/R separation, and product-truth tests. |
