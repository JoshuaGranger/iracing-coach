...
## Current 0.14.0 evidence

Integrated automated runs recorded 138/138 .NET tests, 187/187 Python tests, four JavaScript syntax checks, and a Release application build with zero warnings and zero errors. They cover session-boundary resets, complete-lap evidence gates, the dedicated live-reader path, bounded telemetry projection/backpressure, popout focus/lifecycle, exact phase/group Race/Qualifying selection and caches, concurrent schema-2 backend migration, serialized same-path durable-state writes with temporary-file cleanup, comparable-lap filtering, pit/run association, missing-channel preservation, corner summaries, catalog parity, and versioned grading. A synthetic 240 Hz reader check is scheduler/structure evidence only; it is not a real SDK or 244 Hz display result. Exact artifact hashes remain pending in `06-reality/implementation-snapshot-0.14.0.md` until the final package is built and measured. Historical privacy scans and guarded installer lifecycle results remain in the applicable `companion-app/RELEASE_*.md` files; unchanged upgrade/uninstall behavior was not rerun for this focused corrective release. No real-telemetry, 60 Hz source-capture, 244 Hz presentation, or user acceptance is claimed here.
