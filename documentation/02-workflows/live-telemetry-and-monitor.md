...
Version 0.13.0 source uses equal `fr`/Star grid tracks in the full page and native popout, a full-height non-shrinking editor with an independently hideable overlay Toolbox, and a display-only native popout with layout selection and machine-local physical scale. Full-page tile trends are timestamped canvas paths. That renderer schedules paint with `requestAnimationFrame` and moves an existing path between real samples; it does not increase the SDK sampling rate or synthesize new Y values. Categorical values use step paths. Positioned missing samples remain explicit gap markers in both seed history and live append, are excluded from range calculations, and split the rendered path instead of becoming zero or a connecting line.

The current native popout updates existing number/bar/gauge/chart elements instead of reconstructing the complete grid on the former 200 ms timer. Its retained trend history is painted through the display dispatcher. This is an implementation-path statement only: neither the JavaScript animation-frame loop nor the native dispatcher proves 60 Hz SDK capture, 244 Hz delivery, or a particular frame rate on the user's hardware.
