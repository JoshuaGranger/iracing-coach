...
Version 0.13.0 source implements the expanded race-browser summary columns, compact Race header, default Telemetry/Race review split, responsive three-pane telemetry workspace, and a portable configurable trace layout capped at ten rows. The trace-layout validator permits any different secondary signal, while rejecting only a duplicate of the primary. The Toolbox supplies draggable signal chips plus primary/secondary drop targets and retains selects for non-drag operation. Every signal uses its own selected-lap range in a shared row, with the primary solid and the secondary dashed; real values and units remain in the cursor card. When vehicle sideslip is unavailable but Yaw Rate is present, Yaw becomes the sole solid/full-opacity row trace rather than remaining styled as a dashed secondary.
