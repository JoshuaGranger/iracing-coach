...
| `RA-036` | The rail MUST integrate run boundaries, flag mix, pace, fuel, and measured tire context. Dense secondary run detail MAY use one keyboard-accessible popover rather than a separate page section. The pit popup MUST open only from the PIT badge, float outside the rail beside the pointer, and show only supported service facts such as confirmed changed tires, recorded per-corner wear, recorded fuel added, service time, estimated fuel range/sufficiency, race laps remaining, damage-repair time, and penalty-service time; an unavailable field MUST NOT be guessed. |
