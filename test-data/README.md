...
Regenerate them from the repository root with `powershell.exe -NoLogo -NoProfile -ExecutionPolicy Bypass -File .\tools\generate-fixtures.ps1`. Golden frontend snapshots should be derived from these files, not from the private `data/` directory.
...
Run `python -X utf8 .\tools\mcp_e2e_smoke.py` to start the real stdio backend with `test-data/ibt` as its trusted source root and a temporary archive, then execute a dashboard call and complete analysis of `synthetic-race.ibt`. It requires no private telemetry.
