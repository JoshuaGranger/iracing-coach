...
Repository-level schemas in `contracts/`, sanitized fixtures in `test-data/`, and `tools/verify-repository.ps1` accompany this reference. Treat those checked-in files and current backend source as authoritative; do not substitute an older installed-plugin cache.
